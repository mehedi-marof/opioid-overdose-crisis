...
(function () {
  ...
  const SECTION_ORDER = ["Race & ethnicity", "Age", "Sex"];
  ...
  d3.csv("data/05_demographics.csv", d => ({
    ...
    r2019: +d.r2019,
    r2024: +d.r2024,
    change: d.change_abs_2019_2024 === "" || d.change_abs_2019_2024 == null
      ? (+d.r2024 - +d.r2019)
      : +d.change_abs_2019_2024
  })).then(rows => {
    const sections = SECTION_ORDER.map(sectionName => ({
      heading: sectionName,
      rows: rows
        .filter(d => d.section === sectionName)
        .sort((a, b) => d3.ascending(a.display_order, b.display_order))
  ...
  function drawGrid(g, x, sections, rowH, headerH) {
    ...
    sections.forEach((section, index) => {
      chartHeight += headerH;
      chartHeight += section.rows.length * rowH;
      if (index < sections.length - 1) chartHeight += 16;
    });
  ...
  function drawRow(g, row, x, yPos, rowH) {
    ...
    // Small arrow showing direction toward 2024
    g.append("path")
      .attr(
        "d",
        `M ${x(row.r2024) - 7 * direction} ${cy - 4}
         L ${x(row.r2024)} ${cy}
  ...
  function drawAxisLabel(g, x, yPos) {
    const axis = g.append("g")
      .attr("transform", `translate(0, ${yPos + 8})`);
  ...
  function fmtChange(value) {
    ...
    return `${Math.abs(value).toFixed(1)} per 100,000 ${direction} than 2019`;
  }

  function showTip(event, d) {
    const html =
      `<div class="tip-name">${d.group}</div>` +
      `<div class="tip-row"><span>2019</span><span class="tip-val">${fmtRate(d.r2019)} per 100,000</span></div>` +
      `<div class="tip-row"><span>2024</span><span class="tip-val">${fmtRate(d.r2024)} per 100,000</span></div>` +
      `<div class="tip-row"><span>Change</span><span class="tip-val">${fmtChange(d.change)}</span></div>`;
